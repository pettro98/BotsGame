...
namespace game_module
{
	...
	- В классе реализованно API для создания ботов.
	- Другое имя класса, используемое в докуметации - контроллер.
	*/
	class Controller
	{
	private:
		/*!
		\brief Указатель на объект позволяющий получать и изменять необходимые данные игры.
		*/
	...
	public:
		...
		*/
		...
		\param my_access Указатель на объект, которым инициализируется поле MyAccess объекта класса.
		*/
		...
		*/
		...
		- Для расчёта дистанции используется функция get_distance. 
		...
		*/
		...
		\param move_points Кол-во очков передвижения, которое даётся юниту на перемещение.
		Значение по умолчанию Army::move_points().
		*/
	...
	private:
		/*!
		\brief Метод возвращает указатель на юнит, находящийся в гексе с переданными координатами.
		- В случае если гекса с такими координатами нет возвращает nullptr.

		\param hex Координаты гекса чей юнит запрашивается.
		*/
		Unit * get_unit(const Pair & hex) const;
		/*!
		\brief Метод возвращает указатель на столицу области, в которой находится гекс
		с переданными координатами.
		- В случае если гекса с такими координатами нет возвращает nullptr.

		\param hex Координаты гекса чья столица области запрашивается.
		*/
		Capital * get_capital(const Pair & hex) const;
		/*!
		\brief Метод проверяет кол-во столиц в области, в которой находится гекс
		с переданными координатами и корректирует его.

		\param hex Координаты гекса, находящегося в интересующей области.
		*/
		void correct_capital(const Pair & hex);
		/*!
		\brief Метод возвращает все гексы, находящиеся в областях, 
		в которых необходимо провести коррекцию столиц из предположения, что гекс 
		с переданными координатами только что сменил владельца с переданным цветом на нового.

		\param hex Координаты гекса, который только что сменил владельца.
		\param color Цвет предыдущего владельца гекса.
		*/
		std::vector<std::set<Pair>> hexs_to_correct(const Pair & hex, hex_color color) const;
		/*!
		\brief Метод обновляет доход переданной области.

		\param district Ссылка на вектор с гексами области.
		*/
